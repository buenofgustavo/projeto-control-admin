...
export class CadastroColaboradorService {
  ...
  private apiUrl4 = `${API_CONFIG.baseUrl}/combinacao-salarial/cadastrar`; // Use a URL da API a partir da configuração
  cadastrarCombinacaoSalarial(combinacaoSalarial: CombinacaoSalarial): Observable<CombinacaoSalarial> {
    if(!this.authToken){
      throw new Error('Token JWT não encontrado, refaça o Login!');
    }
    const headers = new HttpHeaders().set('Authorization', `Bearer${this.authToken}`);
    return this.http.post<CombinacaoSalarial>(this.apiUrl4, combinacaoSalarial, { headers });
