...
public class ExportController {
    ...
    // DP
    @GetMapping("/dp/{mes}/{ano}")
    public ResponseEntity<?> exportExcelVariaveisDP(HttpServletResponse response,
                                                    @PathVariable String mes,
                                                    @PathVariable Integer ano) throws IOException
    {
        System.out.println(mes + ' ' + ano);
        exportService.exportVariaveisDP(response, mes, ano);
        return ResponseEntity.status(HttpStatus.OK).body("Exportação para Excel iniciada com sucesso: " + mes + ", " + ano);

    }

    @GetMapping("/dp-lancamento/{mes}/{ano}")
    public ResponseEntity<?> exportExcelLancamentosDP(HttpServletResponse response,
                                                    @PathVariable String mes,
                                                    @PathVariable Integer ano) throws IOException
    {
        System.out.println(mes + ' ' + ano);
        exportService.exportLancamentosDP(response, mes, ano);
        return ResponseEntity.status(HttpStatus.OK).body("Exportação para Excel iniciada com sucesso: " + mes + ", " + ano);
